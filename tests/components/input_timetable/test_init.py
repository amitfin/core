...
async def test_invalid_unset(hass):
    """Test invalid unset method."""
    assert await async_setup_component(hass, DOMAIN, {DOMAIN: {"test": {}}})
    entity_id = "input_timetable.test"
    time = datetime.time.fromisoformat("01:02:03")
    await call_set(hass, entity_id, time, STATE_ON)
    unset_time = datetime.time.fromisoformat("04:05:06")
    with pytest.raises(vol.Invalid) as excinfo:
        await call_unset(hass, entity_id, unset_time)
    assert f"The time {unset_time.isoformat()} doesn't exist" in str(excinfo.value)
...
async def test_invalid_reconfig(hass):
    """Test reconfig method."""
    assert await async_setup_component(hass, DOMAIN, {DOMAIN: {"test": {}}})
    entity_id = "input_timetable.test"
    with pytest.raises(vol.Invalid) as excinfo:
        await call_reconfig(
            ...
            entity_id,
            [
                {ATTR_TIME: "01:02:03", ATTR_STATE: STATE_OFF},
                {ATTR_TIME: "04:05:06", ATTR_STATE: STATE_OFF},
                {ATTR_TIME: "01:02:03", ATTR_STATE: STATE_ON},
            ],
        )
    assert "The same time is not allowed more than once" in str(excinfo.value)
